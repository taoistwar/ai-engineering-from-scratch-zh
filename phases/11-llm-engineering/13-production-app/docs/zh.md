...
> 你已经构建了提示词、嵌入、RAG 流水线、函数调用、缓存层和护栏。分开地，孤立地。像是练习吉他的音阶却从未演奏过一首歌。本课就是那首歌。你将把 01-12 课中的每个组件组合成一个生产就绪的服务。不是玩具，不是演示。而是一个处理真实流量、优雅降级、流式传输 token、跟踪成本并在首批 10,000 用户中存活的系统。
...
构建一个 LLM 功能需要一下午，发布一个 LLM 产品需要几个月。

差距不在于智能，而在于基础设施。你的原型调用 OpenAI，得到回复，打印出来，在你的笔记本上运行。然后现实来临：

- 一个用户发送一个 50,000-token 的文档。你的上下文窗口溢出。
- 两个用户在 4 秒内问同样的问题。你为两者都付费。
- API 在凌晨 2 点返回 500 错误。你的服务崩溃。
- 一个用户要求模型生成 SQL。模型输出 `DROP TABLE users`。
- 你的月账单达到 $12,000，而你不知道是哪个功能导致的。
- 回复时间平均 8 秒，用户在 3 秒后就离开。

今天线上的每个 LLM 应用——Perplexity、Cursor、ChatGPT、Notion AI——都解决了这些问题。不是通过更聪明的提示，而是通过严格的工程实践。

这是顶峰项目（Capstone）。你将构建一个完整的生产级 LLM 服务，集成提示管理（L01-02）、嵌入和向量搜索（L04-07）、函数调用（L09）、评估（L10）、缓存（L11）、护栏（L12）、流式传输、错误处理、可观察性和成本跟踪。一个服务，每个组件连接在一起。
...
请求通过 API 网关进入，网关处理认证和速率限制。输入护栏在提示路由器选择正确模板之前检查提示注入和禁止内容。语义缓存检查类似问题最近是否已回答。如果缓存未命中，LLM 在启用流式传输的情况下被调用。输出护栏验证回复。评估日志记录器记录质量指标。成本跟踪器记录每个 token。响应流式传输回客户端。

七个组件，每个都是你已经完成的课程的组件。工程的难点在于连接它们。
...
sequenceDiagram
    ...
    S-->>C: SSE: data: [DONE]
```
### 流式传输：为什么重要（Streaming: Why It Matters）

一个 GPT-5 的 500 输出 token 回复需要 3-8 秒完全生成。没有流式传输，用户在整个期间盯着一个加载圈。有了流式传输，第一个 token 在 200-500ms 到达。总时间相同，但感知延迟下降 90%。
...
SSE 是默认选择。OpenAI、Anthropic 和 Google 都通过 SSE 进行流式传输。你的服务器从 LLM API 接收 chunks 并将其作为 SSE 事件转发给客户端。客户端使用 `EventSource`（浏览器）或 `httpx`（Python）消费流。

### 错误处理：三层（Error Handling: The Three Layers）

生产级 LLM 应用以三种不同方式失败。每种需要不同的恢复策略。

**第 1 层：API 故障。** LLM 提供者返回 429（速率限制）、500（服务器错误）或超时。解决方案：带抖动的指数退避。从 1 秒开始，每次重试翻倍，添加随机抖动以防止惊群效应。最多 3 次重试。
